...
loaded 128-byte module is a larger loader that pulls in the OS). The mechanism is
described in the manual's Theory of Operation (§2, "Bootstrap is initiated…"): Power-On
Clear (also generated by RESET) sets flip-flop U34, whose not-Q output — passed by S1
position 5 — is the bootstrap signal. That signal gates a row of nine non-inverting
buffers that suspend the CPU's normal control of the data bus and drive the board's own
control signals onto the S-100 status lines, and (with a signal from U37) it activates
U27 during the read cycle so that each fetch reads from the 82S123 PROM while the byte
written to 0000 goes through the bus to regular RAM. The loaded sector therefore does not
clobber the still-running bootstrap. When the bootstrap completes, U43/U34/U37 relinquish
control back to the bus. On any disk error it HALTs (retry via
...
**Altair/S-100 bus lines the board drives while the bootstrap runs.** The board does not
use PHANTOM* — it takes over by disabling the CPU's status buffers and forcing the status
lines itself. Reading the schematic, ten bus pins are involved (definitive names from the
Altair 8800 bus definition in `reference/Altair 8800 Theory of Operation.md`):

| Pin | Signal | Meaning |
|---|---|---|
| 18 | **STA DSB** (active-low) | **Status Disable** — tri-states the CPU's eight status-line buffers so the board can drive them. This is the common enable gate for the takeover, not one of the nine buffers. |
| 44 | SM1 | M1 status — first-byte instruction fetch |
| 45 | SOUT | OUT status — output device address on the bus |
| 46 | SINP | INP status — input device address on the bus |
| 47 | SMEMR | MEMR status — data bus holds memory read data |
| 48 | SHLTA | HLTA status — HALT acknowledge |
| 49 | CLOCK (active-low) | inverted 2 MHz oscillator output |
| 96 | SINTA | INTA status — interrupt acknowledge |
| 97 | SWO (active-low) | WO status — write-memory / output cycle |
| 98 | SSTACK | STACK status — stack-pointer address on the bus |

Pin 18 (STA DSB) is the status-disable gate. The other nine — all eight status lines (SM1,
SOUT, SINP, SMEMR, SHLTA, SINTA, SWO, SSTACK) plus CLOCK (pin 49) — are the outputs the
board forces, and they are exactly the "row of nine non-inverting buffers" the Theory of
Operation describes, gated onto the bus while STA DSB holds the processor's own status
outputs off. The board thus silences the processor's status lines and substitutes its own,
so each fetch is satisfied from the 82S123 PROM while the byte written to 0000 still lands
in regular RAM.
...
same 8-port block (default **F8**), the same 32-byte boot PROM and bootstrap mechanism,
...
The board boots its PROM the same way the single-density card does (see §7) — no dedicated
memory address is occupied. It runs at **2 or 4 MHz** with a **250/500 kbit/s** media rate
selected by the density bit. The
