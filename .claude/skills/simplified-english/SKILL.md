...
| **W11** | Make every "it", "this" and "that" point at something in the same or the last sentence. If not, repeat the noun. |
...
2. For each section, decide: strict or light (§1).
3. Report the findings **grouped by rule ID**. Give `file:line` for each one. When a rule
   fails in many places, give the worst five and the count.
4. Show a before and an after for the three worst passages.
5. Report factual errors that you find on the way in a separate list. An unrelated error
...
- After a rewrite, check it: count the words in each sentence that looks long (W1), and
  search for dashes (W9) and for the left column of §3.
