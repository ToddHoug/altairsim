...
// Rev 1 (`sior1`) -- A at the profile's own 0/1, B the same shape with its ports overridden
// to 2/3. CONFIG SAVE writes `profile` before the port overrides, so `[board.unit.b]
// profile="sior1"` + status_port=2 / data_port=3 reloads to exactly 2/3.
GsioBoard::GsioBoard() {
    ...
    addChannel("a", a, "sior1");

    SerialStraps b = a;
    b.statusPort = 0x02;
    b.dataPort   = 0x03;
    addChannel("b", b, "sior1");  // report `sior1`; the 2/3 port overrides round-trip
}
