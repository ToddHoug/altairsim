#pragma once
//
// SerialBoardBase -- the reusable 88-SIO B serial engine, and NOT a registered board.
//
// One COM2502 UART, the INVERTED SIO status word (Rev0/Rev1), the two interrupt-enable
// flip-flops, the VI0-VI7 straps, the card's own refresh()/nextEdge() clock, the
// connector and the snapshot. Everything an 88-SIO B PCB does between the chip's pins
// and the S-100 bus lives here, because three shipping cards carry that same PCB:
//
//   * the 88-SIO itself (boards/mits-88sio.h) -- this engine, plainly, with a connector;
//   * the 88-ACR (boards/mits-88acr.h) -- the same SIO B with a modem soldered to it,
//     via CassetteBoardBase, which extends this;
//   * the 88-UIO (boards/mits-88uio.h) -- an 88-ACR cassette section plus a 6850 serial
//     section, also via CassetteBoardBase.
//
// The manual settles that this is one PCB and not three lookalikes: "The 88-ACR consists
// of two separate PC boards... One of these is the ACR Modem Board and the other is the
// 88-SIO B" -- and it reprints the whole 88-SIO manual as the ACR's own chapter, status
// word and all. So the status word, the inversion, the VI straps and the decode are
// written ONCE, here, rather than copied into three cards that would drift the day one of
// them is fixed and the others are not.
//
// ABSTRACT. It never answers type() (Board::type() stays pure virtual), so it cannot be
// instantiated or registered on its own -- a concrete card names itself. A change here
// reaches all three cards ON PURPOSE; that is what "shared engine" means. What the split
// prevents is the opposite leak: a quirk of the CONCRETE 88-SIO (or 88-ACR, or 88-UIO)
// can no longer reach the others, because none of them is built on another.
//
// It is NOT a 6850. There is no control register in the 6850 sense: the only thing
// software can write to the control channel is the two INTERRUPT ENABLES. Word format
// and baud are PADS ON THE PCB -- soldered, not programmable -- which is why they are
// board properties here and control-register bits on the 2SIO.
//
// Two ports: a control/status channel at an EVEN address and a data channel at the odd
// address above it. Any even address from 000 to 376 octal.
...
class SerialBoardBase : public Board {
public:
    ~SerialBoardBase() override;
    ...
    // flip-flops on the card (IC B). The straps and the port are config. The cassette
    // engine extends this. deserialize() re-arms via refresh() from the restored state.
    void serialize(StateWriter& w) const override;
    void deserialize(StateReader& r) override;

    // One UART. The cassette engine inherits this override with it -- a cassette
    ...
    // ---- PROTECTED, FOR THE CASSETTE ENGINE AND THE CONCRETE CARDS ---------------
    //
    ...
    //
    // So CassetteBoardBase derives from this one, and the status word below is written
    // ONCE. That is not the "shared UART helper with a bool invert" that the .cpp warns
    // about at length -- that trap is about the 88-2SIO, a DIFFERENT card with a
    // DIFFERENT chip whose bits are the other way up. The trap HERE is the opposite one:
    // several copies of the SAME PCB, which drift the day somebody fixes a bug in one
    // status word and not the others, and then the machine has three different 88-SIO Bs
    // in it.
    //
...
protected:
    SerialBoardBase();   // -> a DEAD line; a concrete card straps or relines from there
