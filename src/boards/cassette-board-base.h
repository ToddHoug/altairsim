#pragma once
//
// CassetteBoardBase -- the reusable MITS audio-cassette engine, and NOT a registered board.
//
// AN 88-SIO B WITH A MODEM BOLTED TO IT, AND THAT IS NOT AN ANALOGY. The 88-ACR manual's
// first sentence:
//
...
// flip-flops. So this engine DERIVES from SerialBoardBase and inherits every one of them
// rather than growing a second, drifting copy. What it ADDS is the cassette:
//
//   * THE STRAPS ARE SPECIFIED. The 88-SIO's jumpers are whatever you soldered; the
//     cassette kit tells you exactly what: "wire address select for 006. Wire BAUD Rate
//     for 300 (max.), and wire UART options for 8 data bits, one stop bit, no parity
//     bit." So the ctor here straps port 0x06, 300 baud, 8N1, Rev 1 -- the MITS cassette
//     section's defaults, shared by the 88-ACR and the 88-UIO's cassette half alike.
//
//   * THE LINE IS SOLDERED TO A CASSETTE. There is no connector to CONNECT to. The
//     UART's serial pins go to the modem, the modem's audio goes to the recorder, and so
//     the unit is a TAPE you MOUNT -- UnitKind::Tape rather than UnitKind::Serial.
//
//   * A TAPE HAS A POSITION, so the engine brings verbs: WIND / REWIND / EXTRACT
//     (Board::commands()). A disk does not need them -- you can seek a disk.
//
// TWO SHIPPING CARDS carry this engine as SIBLINGS, neither built on the other:
//   * the 88-ACR (boards/mits-88acr.h) -- this engine, plainly, one MITS modem;
//   * the 88-UIO (boards/mits-88uio.h) -- this engine plus a 6850 serial section, a
//     motor relay and an SW-1 that selects MITS or Kansas City modulation.
//
// ABSTRACT. It never answers type() (Board::type() stays pure virtual), so it cannot be
// instantiated or registered on its own. A change here reaches both cassette cards ON
// PURPOSE; what the split prevents is a quirk of the concrete 88-ACR reaching the 88-UIO,
// which used to be built on it.
//
// WHAT THE MODEM CONTRIBUTES TO THE REGISTER MODEL: NOTHING. It is analog. Two tones
// -- 2400 Hz for a logic 1, 1850 Hz for a logic 0, FSK -- and a PLL to get them back.
// The guest cannot observe a single bit of it, which is precisely why this is short.
//
// AND THERE IS NO MOTOR CONTROL on the plain cassette. Not "we did not model it" -- the
// 88-ACR HAS none; the operator pressed PLAY with a finger. The 88-UIO adds a relay, and
// it does so in its own class, not here.
...
class CassetteBoardBase : public SerialBoardBase {
public:
    // The 88-SIO surface, MINUS `connect`. THE CARD HAS NO CONNECTOR: its UART's serial
    // pins are soldered to the modem board. Offering an endpoint would advertise a socket
    ...
    // the SIMULATOR because it is not on the card in REALITY: the plain cassette has no
    // motor control, no transport register, and no way to know or care which button is
    ...
    // WIND (and REWIND, its wind-to-start alias) and EXTRACT. The reason Board::commands() exists.
    ...
    // this engine's two runtime facts: which button is down (mode_) and where the head
...
protected:
    CassetteBoardBase();   // straps the MITS cassette section: 0x06, 300 baud, 8N1, Rev 1

    // WHAT THIS CARD'S MODEM CAN PHYSICALLY HEAR -- one modulation, because a plain
    // cassette card has one modem: continuous FSK at 2400/1850, 300 baud. A Kansas City
    // tape is REFUSED rather than decoded, because the real PLL sits at 2125 Hz and takes
    ...
    //
    // VIRTUAL, because a concrete card can carry a different modem -- the 88-UIO
    // (mits-88uio.h) has a SW-1 that selects MITS 2400/1850 OR Kansas City 2400/1200,
    // so its override returns the one the switch is set to. That is not inventing
    // hardware: it is a switch the physical UIO board has and the ACR does not. The
    // default answer is unchanged -- {fsk300_1850()} and nothing else.
...
private:
    ...
    // Sol deck's `rc`, does not apply here: the FSK is not a clock-divider tone --
