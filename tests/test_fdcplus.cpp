...
void test_fdcplus() {
    SECTION("FDC+ -- four ports at 08 (or 80), the fourth reads 00; drivetype 6|7; no MOUNT");
    {
        Rig r;
        BusCycle c;
        c.type = Cycle::IoRead;
        ...
        CHECK(!setProperty(r.b, "drivetype", "5", err), "drivetype 5 is not this board (yet)");
        CHECK(!setProperty(r.b, "drivetype", "8", err), "nor is 8");
        CHECK(!r.b.mount("drive0", "x.dsk", false, err), "MOUNT refused: the server mounts images");
