...
Publish `SHA256SUMS` alongside the four so the two locations can be checked against each other, and so anyone downloading can check what they got. **`tools/build-checksums.sh` produces it** (§5 step 6): run on the coordinator once `dist/` holds all four, it writes the standard `shasum -a 256`/`sha256sum` format — bare names, so `shasum -a 256 -c SHA256SUMS` verifies from inside `dist/` or an unpacked download alike — and refuses a partial file rather than vouch for three of four.
