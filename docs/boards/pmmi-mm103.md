...
**Deferred to later milestones:** interrupts (the enable bit and mask staging are shadowed but inert), and the pulse dialer's digit decoding. **There is no dialer by design** — SH is a relay bit and nothing decodes pulses into a number; the far end is `dial=` host config, not something the guest produces (see *How it is simulated*).
