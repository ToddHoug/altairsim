...
- Properties: `port`, `dial`, `answer`, `rtsdtr`. `interrupt` and `carrier_delay` belong to later milestones. **Self-test is *not* a property** — it is guest-controlled by `OUT BA+3` bit 4, not an operator jumper, so it is surfaced read-only in the `lines` status string (capital `ST` when looped) rather than as a setter.
