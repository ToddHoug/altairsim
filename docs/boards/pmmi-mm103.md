...
**The modem handshake and the software-controlled phone line are real.** Two board properties — **`dial=host:port`** and **`answer=port`** — turn the one serial line into a `ModemLine` (a dynamic phone line over TCP, `src/host/modemline.{h,cpp}`). The guest's `OUT BA+0`/`OUT BA+3` writes drive it: **SH off-hook + DTR originates** the call to `dial=`. With **`answer=`** the phone line is **plugged in for the life of the machine** — an inbound connection *rings* whether or not DTR is high, so answer-mode software (CBBS) that waits for a ring with the modem **on-hook** (DTR low) hears the call, and picks it up by raising DTR and setting answer mode (RI). **DTR-drop hangs up the current call but leaves the line listening**, so the next caller is answered rather than refused; a caller who drops before being answered is cleaned up promptly. Because the far end of an answering modem is almost always a person's telnet client, a `dial=`/`answer=` line **speaks the Telnet protocol by default** — so a stock `telnet` client drops its local echo and sends one key at a time, with no double echo. Set **`telnet=off`** for a raw pipe (a modem link to another simulator). `IN BA+2` is no longer a constant — it is computed from the phone line plus a **Clock-driven handshake state machine** (ringing bursts, dial tone, the billing/CTS/AP timings below). See *How it is simulated* and `tests/test_pmmi.cpp`.

**A real serial port is a straight passthrough.** `CONNECT pmmi0:line serial:/dev/tty…` (or `serial:COM3`) puts an actual host serial port on the line instead of a phone line, and the card's registers drive the wire directly: the guest's **DTR** (`OUT BA+3`) reaches the port's DTR pin, and the port's live **CTS / DCD / RI** come back in `IN BA+2` (CTS→CTS, RI→Ringing, DCD→Answer-Phone/off-hook). The software-programmed **baud and frame** (`OUT BA+0`/`OUT BA+2`) reprogram the real port, exactly as they would on the bench. This is the "modem signals are uniform" rule — the board reads the pins it has; a file or socket asserts them all and so still reads the fixed `0x43` "ready" it always did. One knob is added for the cable that needs it: **`rtsdtr=on`** makes RTS follow DTR (default off).

**Not modelled:** interrupts — the enable bit and mask staging are shadowed but **inert**, because the E1–E7 pad → VI0–VI7 correspondence is the one fact the manual does not give (`DESIGN.md` §17) — and the pulse dialer's digit decoding. **There is no dialer by design**: SH is a relay bit and nothing decodes pulses into a number; the far end is `dial=` host config, not something the guest produces (see *How it is simulated*).
...
- The phone line is a **`ModemLine`** (`src/host/modemline.{h,cpp}`) — a `ByteStream` that, unlike a static `socket:` endpoint, holds **no sockets when idle** and opens/closes them as the guest drives the modem. It is built only when `dial=` or `answer=` is set; with neither, the line stays a `NullStream` and `IN BA+2` reverts to the fixed "ready" stub.
- **The two knobs are `dial=host:port` and `answer=port`.** `dial=` is the far end an originate call reaches; `answer=` is the TCP port an auto-answer call is accepted on. Both are validated through the endpoint grammar (`parseHostPort`/`parsePort`), never parsed by the board itself. `SHOW pmmi0` and `CONFIG SAVE` round-trip them, and the line advertises itself as `modem:dial=…,answer=…`.
- **Originate:** the guest goes off-hook (`OUT BA+0` SH=1) to hear a synthetic **dial tone** (`IN BA+2` bit 0 = 0), then enables the modem (`OUT BA+3` DTR=1) — *that* is when the `dial=` socket is opened. Carrier is the established TCP session; **CTS clears 750 ms after carrier** (§10.3).
- **Answer:** raising **DTR arms the listener** on `answer=` (the only thing that opens a socket on an idle modem — so "idle = no sockets" is literally true). An inbound connection **rings**: `IN BA+2` bit 1 toggles between bursts so the guest counts rings, and **not one byte crosses until the guest answers** by setting RI (`OUT BA+0` bit 1). Then AP goes off-hook, the **2 s billing delay** holds, and **CTS clears 450 ms** after carrier (§8/§10.2).
- **SH is a relay pin.** The make/break pulses a guest's dialer produces are *not* decoded into digits, and no property maps a dialed number onto a host and port — the far end is `dial=` host config. Reading a number out of the hook relay and steering it at an IP address would be a behavior this card never had; the MM-103 did not know what it was dialing either. A period dialer still runs, still pulses the hook (the timer-pulse bit toggles so its timing loop advances), and still works in the only sense the hardware ever offered. **DTR is the disconnect** (`OUT BA+3` DTR→0 hangs up and drops all sockets), not SH — a correct guest clears SH after CTS, and AP holds the line (§7.4.4.5).
- Timer pulses and all 6860 timing come from the **`Clock`**, in T-states, so `REPLAY` stays deterministic. A **live call cannot be snapshotted** (a socket handle is not serializable): `RESTORE` brings the modem back **on-hook** and the guest redials — the honest analogue of pulling the line during a save.
- **A CONNECTed serial port (or socket/file) is a passthrough, not a phone line.** When the line is a plain endpoint rather than a `ModemLine`, `IN BA+2` is folded straight from the stream's pins (`host/stream.h` `LineStatus`): `CTS→CTS`, `RI→Ringing`, `DCD→Answer-Phone` (off-hook); Dial-Tone reads absent (a wire has none) and Mode/Timer stay board-generated. On a real `serial:` port those are live RS-232 pins; a file/socket asserts them all, so the byte is the familiar `0x43`. The guest's **DTR** (`OUT BA+3` bit 6) is driven onto the port via `ByteStream::setControl`; **RTS stays low unless `rtsdtr=on`**, which mirrors it onto DTR for cables that need it. Baud/frame reprogram the real port through the same `setParams` path the 88-2SIO uses. This is the one place SH/DTR reach real hardware; the `ModemLine` path (dial=/answer=) is unchanged and its disconnect control is DTR alone.
- Properties: `port`, `dial`, `answer`, `rtsdtr`, `telnet`. `telnet` (default **on**) makes a `dial=`/`answer=` line speak the Telnet protocol so a human telnet client behaves; `telnet=off` is a raw pipe. It rides through the shared `TelnetCodec` (`src/host/telnet_codec.{h,cpp}`), the same engine the `telnet:` endpoint uses, and applies only to the phone line — a CONNECTed endpoint uses the `telnet:` endpoint instead. `interrupt` and `carrier_delay` are not properties yet — see *Not modelled* above. **Self-test is *not* a property** — it is guest-controlled by `OUT BA+3` bit 4, not an operator jumper, so it is surfaced read-only in the `lines` status string (capital `ST` when looped) rather than as a setter.
...
- **Dial-tone detection is synthetic** — asserted while the modem is off-hook to originate (SH set, `dial=` configured) and the call is not yet up. There is no real dial tone to detect.
- **Dialed digits are not decoded.** The hook relay follows SH faithfully, but nothing counts the pulses and no number selects a far end — the far end is the `dial=` host config. This is a deliberate limit, not a gap waiting to be filled: decoding a phone number and resolving it to a host would be an invented feature.
- **The handshake numbers are the reference values, coarsely applied.** The billing (2 s), CTS (450 ms answer / 750 ms originate), AP reset (~1.5 s) and 17 s no-handshake timeout are real and Clock-driven, but the ≥ 51 ms SH/RI hold, the exact ring cadence, and the space-disconnect timers are approximated — a polling guest tolerates coarse values. Rx Break latching (`IN BA+2` bit 3) is not modeled.
- **A live call does not survive `SNAPSHOT`/`RESTORE`.** A socket handle is not serializable, so restore returns the modem on-hook (AP high, no CTS) and the guest redials. The `dial=`/`answer=` config travels through the machine file, not the snapshot.
- **Self-test / loopback is honored** on the data path. `OUT BA+3` with DTR set (bit 6) and ST asserted (bit 4 = 0) patches a loopback plug onto the UART — transmitted characters return on receive, and the modem control pins loop too (RTS→CTS, DTR→DCD/DSR), exactly as a bench loopback plug does. The real phone line is pocketed while looped and restored, intact, when ST clears (or on reset). The loopback is transient runtime state; it is re-derived from the restored control latch after a `RESTORE`. Two documented bounds are **not** enforced, matching the "model the effect, not the fiction" split: the manual requires **answer mode** and a **disconnected line** for a meaningful self-test, and neither is checked; and while looped, `IN BA+2` reverts to the fixed "ready" constant (so the **MODE bit** is not flipped by ST) rather than reporting the handshake state machine — the loopback is a data-path diagnostic, not a modem session.
